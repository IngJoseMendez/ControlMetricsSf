...
export default function SPCHome() {
  return (
    <div className="p-8">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900">Panel de Módulos SPC</h1>
        <p className="text-gray-500 mt-1">Seleccione un módulo para iniciar el análisis estadístico.</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">
        {modules.map(({ href, label, icon: Icon, color, desc }) => (
          <Link key={href} href={href}
            className="group flex items-center gap-5 bg-white border border-gray-100 rounded-2xl p-5 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">
